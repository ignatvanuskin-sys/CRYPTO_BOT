...
async def cmd_start(message: Message, session):
    ...
    if user.phone_verified_at is None:
        ...
        await message.answer(
            f"{tg_emoji(PARTY_ID, '🎉')} <b>Привет! Это турнир по криптотрейдингу</b>\n\n"
            f"Торгуй криптой по <b>реальным ценам BingX</b> — все деньги виртуальные.\n\n"
            f"{tg_emoji(GOLD_ID, '🥇')} <b>Топ-10 трейдеров</b> в конце недели делят призовой фонд\n"
            f"{TG_MONEY} Каждый начинает с одинакового старта — <b>$10 000</b>\n"
            f"{tg_emoji(CHART_UP_ID, '🚀')} LONG на росте, SHORT на падении, плечо до x300\n\n"
            f"{tg_emoji(PIN_ID, '📌')} <b>Чтобы участвовать — нажми кнопку ниже</b> 👇\n"
            f"На твой аккаунт зачислится <b>$10 000 виртуальных долларов</b>, и ты сразу в игре!\n\n"
            f"<i>Все деньги виртуальные — это не биржа, риск потери реальных средств отсутствует.</i>",
